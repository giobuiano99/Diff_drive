...
void goal_gen::loop() {
	...
	while (!sent && ros::ok()) {
		try{
			_listener.waitForTransform("camera_frame", "aruco_marker_frame", ros::Time(0), ros::Duration(60.0));
			...
			_listener_goal.waitForTransform("map", "goal_frame", ros::Time(0), ros::Duration(60.0));
